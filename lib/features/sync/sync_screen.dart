...
class _SyncScreenState extends State<SyncScreen> {
  ...
  Future<void> _run(Future<void> Function() action) async {
    ...
    try {
      ...
      // Supabase AuthException has a readable message via toString fallback.
      if (_sync.message != null && _error == null) _error = _sync.message;
      _error ??= 'Something went wrong. Check your email/password and try again.';
    } finally {
        ...
        builder: (context, _) {
          return SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: sync.signedIn ? _signedIn(sync) : _signedOut(sync),
            ),
          );
        },
      ),
  ...
  Future<void> _signIn() =>
      _sync.signIn(_email.text, _password.text);
    ...
    return Column(
      ...
      children: [
        ...
        const Text(
          ...
          'one device appear on the other within a few seconds (while online).',
  ...
  (IconData, String, Color) _statusBits(SyncService sync) {
    switch (sync.state) {
      ...
      case SyncState.synced:
        return (Icons.check_circle_outline, 'Up to date', const Color(0xFF2E7D32));
